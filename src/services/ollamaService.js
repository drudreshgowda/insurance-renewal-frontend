...
const checkOllamaStatus = async () => {
  try {
    ...
    if (!availableModels.includes(MODEL_NAME)) {
      throw new Error(`Model ${MODEL_NAME} not found. Available models: ${availableModels.join(', ')}`);
    }
...
const RENEWAL_AGENT_PROMPT = `You are iRenewal, an intelligent AI assistant specialized in insurance renewal data analysis and management. You have a friendly, professional personality and maintain conversation context to provide personalized assistance.

PERSONALITY & BEHAVIOR:
- Your name is "iRenewal" - introduce yourself when appropriate
...
Please provide a well-structured response with proper formatting using the guidelines in your system prompt.`,
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(`Failed to get response from Ollama: ${errorData.error || response.statusText}`);
    }
